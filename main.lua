...
RegisterNetEvent('ty-rental:sv:rentVehicle', function(data)
    local src = source
    ...
    if data.paymentmethod == "cash" then
        if Player.Functions.GetMoney(data.paymentmethod) >= data.payment then
            ...
            TriggerClientEvent('ty-rental:cl:spawnVehicle', src, data.carname, data.renttime)
...
RegisterNetEvent('ty-rental:sv:updatesql', function(plate,vehicle,time)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid = Player.PlayerData.citizenid
...
RegisterNetEvent('ty-rental:sv:checktime', function()
